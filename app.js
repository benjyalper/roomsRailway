...
app.post('/submit', async (req, res) => {
    try {
        ...
        try {
            ...
            await connection.execute('INSERT INTO selected_dates (selected_date, names, color, startTime, endTime, roomNumber, recurringEvent, recurringNum) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [selectedDate, names, selectedColor, startTime, endTime, roomNumber, recurringEvent, recurringNum]);
            ...
            if (recurringEvent) {
                ...
                for (let i = 1; i <= `${recurringNum}`; i++) {
                    ...
                    await connection.execute('INSERT INTO selected_dates (selected_date, names, color, startTime, endTime, roomNumber, recurringEvent, recurringNum) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [nextDate, names, selectedColor, startTime, endTime, roomNumber, recurringEvent, recurringNum]);
                }
            }
